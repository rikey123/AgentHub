...
test.describe("V1.0 settings connectivity", () => {
  ...
  test("models and runtimes settings write through CSRF-protected daemon routes", async ({ page }) => {
    ...
    await page.getByLabel("Name").fill("Local Ollama");
    await page.getByLabel("Model ID").fill("llama3.2");
    ...
    await page.getByRole("tab", { name: /Runtimes/u }).click();
    await expect(page.locator('[data-testid="runtime-card-native-default"]')).toContainText("Connected");
    await page.locator('[data-testid="runtime-card-native-default"]').getByRole("button", { name: "Test Connection" }).click();
    await expect.poll(
