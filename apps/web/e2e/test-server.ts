...
export async function startTestServer(daemon: DaemonApp): Promise<{ server: Server; url: string; close: () => void }> {
  ...
  const server = createServer((req, res) => {
    ...
    const apiPrefixes = ["/auth/session", "/event", "/rooms", "/roles", "/runtimes", "/model-configs", "/agent-bindings", "/settings", "/workspaces", "/agents", "/runs", "/context", "/permissions", "/interventions", "/artifacts", "/deployments", "/deployment-providers", "/api", "/preview", "/sites", "/debug", "/pending-turns", "/messages", "/tasks", "/mailbox", "/skills", "/board", "/timeline", "/scheduler", "/cron", "/recurring-tasks", "/healthz", "/openapi.json"];
