...
export function ContextTab({ room, runId }: { room: RoomViewModel; runId: string }) {
  ...
  if (items.length === 0) {
    return <div className="p-6 text-center text-sm text-muted">这个 Run 暂无上下文。</div>;
  }
  return (
          ...
          <Card variant="transparent" className="border border-border">
            <Card.Header>
              <div className="flex items-center gap-2">
                ...
                <Chip size="sm" variant="soft" color={contextStatusColor(c.status)}>{contextStatusLabel(c.status)}</Chip>
              </div>
