...
import { permissionStatusColor, permissionStatusLabel } from "../../../lib/status.ts";
...
export function PermissionsTab({ room, runId, csrfFetch }: { room: RoomViewModel; runId: string; csrfFetch: typeof fetch }) {
  ...
  if (perms.length === 0) {
    return summary.length === 0 ? <div className="p-6 text-center text-sm text-muted">这个 Run 暂无许可记录。</div> : (
          ...
          <Card key={`${item.modelConfigId}-${index}`} variant="transparent" className="border border-border">
            <Card.Header>
              <div className="flex items-center gap-2">
                ...
                <Chip size="sm" variant="soft" color={item.decision === "allowed" ? "success" : item.decision === "denied" ? "danger" : "warning"}>{permissionStatusLabel(item.decision)}</Chip>
              </div>
