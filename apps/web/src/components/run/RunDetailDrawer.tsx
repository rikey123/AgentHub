    ...
    <Drawer.Backdrop isOpen={props.isOpen} onOpenChange={props.onOpenChange}>
      <Drawer.Content placement="right">
        <Drawer.Dialog className="ah-run-detail-drawer w-[640px] max-w-[90vw]">
          <Drawer.CloseTrigger aria-label="关闭 Run 详情" />
          <Drawer.Header>
            <Drawer.Heading>{run?.agentName ?? "Run"} - {run ? runStatusLabel(run.status) : "未知状态"}</Drawer.Heading>
            {run ? (
          ...
          <Drawer.Body className="p-0">
            {!runId ? (
              <div className="p-6 text-center text-sm text-muted">尚未选择 Run。</div>
            ) : !room || !run ? (
              <div className="flex flex-col gap-3 p-6" aria-label="正在加载 Run">
              ...
              <div data-testid="run-detail-tabs" className="flex h-full min-h-0 flex-col">
                <Tabs selectedKey={selectedTab} onSelectionChange={(key) => setSelectedTab(String(key))} className="flex h-full min-h-0 flex-col">
                  <Tabs.ListContainer className="ah-run-detail-tabs-wrap">
                    <Tabs.List aria-label="Run 详情" className="ah-run-detail-tabs">
                      <Tabs.Tab className="ah-run-detail-tab" id="transcript" data-testid="run-detail-tab-transcript">记录<Chip className="ml-1" size="sm" variant="soft" color="default">{transcriptCount}</Chip><Tabs.Indicator /></Tabs.Tab>
                      <Tabs.Tab className="ah-run-detail-tab" id="tools" data-testid="run-detail-tab-tools">Tools<Tabs.Indicator /></Tabs.Tab>
                      ...
                      <Tabs.Tab className="ah-run-detail-tab" id="raw" data-testid="run-detail-tab-raw">原始流<Tabs.Indicator /></Tabs.Tab>
                      <Tabs.Tab className="ah-run-detail-tab" id="cost" data-testid="run-detail-tab-cost">计费<Tabs.Indicator /></Tabs.Tab>
                    </Tabs.List>
                  </Tabs.ListContainer>
            ...
            {task ? (
              <div data-testid="task-detail-drawer" className="flex flex-col gap-3 text-sm">
                ...
                <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-xs">
                  ...
                  <dt className="font-semibold text-muted">来源 Run</dt>
