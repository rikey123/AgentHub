...
describe("ModelsTab REST integration contract", () => {
  ...
  it("builds Ollama payloads with baseURL and no API key", () => {
    ...
    expect(buildModelConfigPayload({
      mode: "add",
      provider: "ollama",
  ...
  it("tests model calls through REST success, failure, and job polling without EventBus", async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async (input) => {
      ...
      if (path === "/model-configs/mc_success/test") {
        return jsonResponse(200, { ok: true, model: "gpt-4o", latencyMs: 23, inputTokens: 1, outputTokens: 1 });
      }
      ...
      if (path === "/settings/jobs/job_1") {
        return jsonResponse(200, { job: { status: "completed", result: { ok: true, model: "llama3.2", latencyMs: 12, inputTokens: 1, outputTokens: 1 } } });
      }
      return jsonResponse(404, { error: "not_found" });
    });

    await expect(testModelConfig(fetchImpl, "mc_success")).resolves.toMatchObject({ ok: true, model: "gpt-4o" });
    await expect(testModelConfig(fetchImpl, "mc_fail")).resolves.toEqual({ ok: false, error: "invalid_api_key" });
    await expect(testModelConfig(fetchImpl, "mc_poll")).resolves.toMatchObject({ ok: true, model: "llama3.2" });
