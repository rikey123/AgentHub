...
export type RoleGenerationStatus = "idle" | "validating" | "generating" | "completed" | "failed" | "expired";
...
export function RoleGeneratorModal({
  ...
  const canGenerate = description.trim().length > 0 && modelConfigId.trim().length > 0 && status !== "generating" && status !== "validating";
  const canSave = status === "completed" && preview.name.trim().length > 0 && preview.prompt.trim().length > 0 && !saving;
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (completed.status === "completed" && completed.draftJson) {
        ...
        setStatus("expired");
        setError("Draft expired. Please generate again.");
      } else {
        setStatus("failed");
        setError(completed.error ?? "Role generation failed.");
      }
    } catch (err) {
  ...
  const save = async () => {
    ...
    setError(undefined);
    try {
      const saved = await createGeneratedRole(fetchImpl, buildGeneratedRoleInput(preview, job.jobId));
  ...
  return (
    <Modal.Backdrop isOpen={isOpen} onOpenChange={(open) => { if (!open) void cancelAndClose(); }}>
      <Modal.Container size="lg">
        <Modal.Dialog aria-label="Generate role with AI" data-testid="role-generator-modal">
          <Modal.CloseTrigger aria-label="Close role generator" />
          <Modal.Header>
            <Modal.Heading>Generate role with AI</Modal.Heading>
          </Modal.Header>
          <Modal.Body className="grid gap-4">
            <div className="rounded-2xl border border-border bg-surface p-3 text-sm text-muted">
              Describe the responsibility you want, choose a configured model, then review the generated draft before saving it as a real role.
            </div>

            <TextField value={description} onChange={setDescription}>
              <Label className="text-sm font-semibold">Description</Label>
              <TextArea className="min-h-28" placeholder="帮我生成一个擅长前端重构的 reviewer" data-testid="role-generator-description" />
            </TextField>
            ...
            {configs.length === 0 ? <p className="text-xs text-warning" role="alert">Add a model config before generating roles.</p> : null}
              ...
              <Card variant="secondary" className="border border-border">
                <Card.Content className="flex items-start gap-3 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold">Generating draft...</p>
                    <p className="mt-1 text-xs text-muted">{job?.tokenCount ?? 0} tokens</p>
                    {job?.promptFragment ? <pre className="ah-mono mt-2 max-h-28 overflow-auto whitespace-pre-wrap text-xs text-foreground">{job.promptFragment}</pre> : null}
                  </div>
                </Card.Content>
              </Card>
            ) : null}
            ...
            {status === "completed" ? (
              <Card variant="default" className="border border-border">
                <Card.Header>
                  <div>
                    <Card.Title>Draft preview</Card.Title>
                    <Card.Description>Edit the generated role before saving.</Card.Description>
                  </div>
                </Card.Header>
                <Card.Content className="grid gap-3">
                  <TextField value={preview.name} onChange={(value) => setPreview((current) => ({ ...current, name: value }))}>
                    <Label className="text-sm font-semibold">Name</Label>
                    <Input data-testid="role-generator-preview-name" />
                  </TextField>
                  <TextField value={preview.description} onChange={(value) => setPreview((current) => ({ ...current, description: value }))}>
                    <Label className="text-sm font-semibold">Description</Label>
                    <Input data-testid="role-generator-preview-description" />
                  </TextField>
                  <TextField value={preview.prompt} onChange={(value) => setPreview((current) => ({ ...current, prompt: value }))}>
                    <Label className="text-sm font-semibold">Prompt</Label>
                    <TextArea className="min-h-36" data-testid="role-generator-preview-prompt" />
                  </TextField>
                  <TextField value={preview.capabilitiesText} onChange={(value) => setPreview((current) => ({ ...current, capabilitiesText: value }))}>
                    <Label className="text-sm font-semibold">Capabilities</Label>
                    <Input placeholder="code.review, code.edit" data-testid="role-generator-preview-capabilities" />
                  </TextField>
            ...
            {status === "failed" || status === "expired" ? (
              <div className="rounded-2xl border border-danger/40 bg-danger-soft p-3 text-sm text-danger-soft-foreground" role="alert">
                <div className="font-semibold">{status === "expired" ? "Draft expired" : "Generation failed"}</div>
                <div className="mt-1">{error ?? "Try again or write the role manually."}</div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button size="sm" variant="secondary" onPress={() => void generate()} data-testid="role-generator-try-again">Try Again</Button>
                  <Button size="sm" variant="tertiary" onPress={() => void writeManually()} data-testid="role-generator-write-manually">Write Manually</Button>
                </div>
              </div>
            ) : null}

            {error && status !== "failed" && status !== "expired" ? <p className="text-xs text-danger" role="alert">{error}</p> : null}
          </Modal.Body>
          <Modal.Footer className="gap-2">
            <Button variant="tertiary" onPress={() => void cancelAndClose()} data-testid="role-generator-cancel">Cancel</Button>
            <Button variant="secondary" isDisabled={!canGenerate} isPending={status === "validating" || status === "generating"} onPress={() => void generate()} data-testid="role-generator-generate">Generate</Button>
            <Button variant="primary" isDisabled={!canSave} isPending={saving} onPress={() => void save()} data-testid="role-generator-save">Save</Button>
          </Modal.Footer>
        </Modal.Dialog>
      </Modal.Container>
    </Modal.Backdrop>
...
export function buildGeneratedRoleInput(preview: PreviewForm, generationJobId: string): RoleInput & { generationJobId: string } {
  return {
...
export async function startRoleGeneration(fetchImpl: typeof fetch, input: { description: string; modelConfigId: string }): Promise<string> {
  const response = await fetchImpl("/roles/generate", {
    method: "POST",
    credentials: "same-origin",
    headers: { accept: "application/json", "content-type": "application/json" },
    body: JSON.stringify({ description: input.description.trim(), modelConfigId: input.modelConfigId.trim() })
  });
  ...
  if (typeof payload?.jobId !== "string" || payload.jobId.length === 0) throw new Error("Role generation response did not include jobId.");
  return payload.jobId;
}

export async function getRoleGenerationJob(fetchImpl: typeof fetch, jobId: string, signal?: AbortSignal): Promise<RoleGenerationJob> {
  ...
  if (response.status === 404) {
    return { jobId, status: "expired", promptFragment: "", tokenCount: 0, error: "Draft expired. Please generate again." };
  }
...
export async function pollRoleGenerationJob(
  fetchImpl: typeof fetch,
  jobId: string,
  options: { intervalMs?: number; signal?: AbortSignal; onUpdate?: (job: RoleGenerationJob) => void } = {}
...
export async function createGeneratedRole(fetchImpl: typeof fetch, input: RoleInput & { generationJobId: string }): Promise<RoleConfig> {
  ...
  if (!role) throw new Error("Generated role response did not include a role.");
  return role;
}

export async function deleteRoleGenerationJob(fetchImpl: typeof fetch, jobId: string): Promise<void> {
  const response = await fetchImpl(`/roles/generate/jobs/${encodeURIComponent(jobId)}`, {
    method: "DELETE",
    credentials: "same-origin",
    headers: { accept: "application/json" }
  });
  if (!response.ok && response.status !== 404) throw new Error(await responseError(response, "Delete role generation job failed"));
}
...
function normalizeDraft(value: unknown): RoleDraftPreview | undefined {
  ...
  return {
    ...
    prompt,
    capabilities: normalizeCapabilities(value.capabilities),
    permissionProfileId: stringField(value.permissionProfileId ?? value.permission_profile_id ?? value.suggestedPermissionProfileId ?? value.suggested_permission_profile_id) ?? null
  };
}
...
async function responseError(response: Response, fallback: string): Promise<string> {
  ...
  return isRecord(payload) && typeof payload.error === "string" ? payload.error : `${fallback}: HTTP ${response.status}`;
}
