...
export function SkillsTab({ skills: initialSkills, runtimes: runtimeData, fetchImpl = fetch, onSkillsChange }: SkillsTabProps) {
  ...
  const [selectedRuntimeId, setSelectedRuntimeId] = useState<string>(() => runtimeOptions[0]?.id ?? "");
  ...
  useEffect(() => {
    ...
    if (!runtimeOptions.some((runtime) => runtime.id === selectedRuntimeId)) setSelectedRuntimeId(runtimeOptions[0]?.id ?? "");
  ...
  const selectedLocalSkill = selectedLocalKeys.size === 1 ? localSkills.find((skill) => selectedLocalKeys.has(skill.key)) : undefined;
  ...
  const openNewSkill = () => {
    setDraft({ mode: "create", name: "new-skill", description: "Describe what this skill helps with.", content: DEFAULT_SKILL_CONTENT, files: [], selectedPath: SKILL_MD_PATH, newFilePath: "" });
  ...
  const saveDraft = async () => {
    ...
    setMessage(undefined);
    try {
      const input = { name: draft.name.trim(), description: draft.description.trim(), content: draft.content, files: draft.files };
      const saved = draft.mode === "edit" && draft.id
  ...
  const importSelectedLocalSkills = async () => {
    ...
    try {
      ...
      for (const skill of localSkills.filter((candidate) => selectedLocalKeys.has(candidate.key))) {
        ...
        imported.push(await importRuntimeLocalSkill(fetchImpl, selectedRuntimeId, {
          skillKey: skill.key,
          name: edit?.name ?? skill.name,
          description: edit?.description ?? skill.description ?? `Imported from ${localResponse?.provider ?? selectedRuntimeId}`
        }));
      }
...
function RuntimeSelect({ runtimes, value, onChange }: { runtimes: readonly RuntimeConfig[]; value: string; onChange: (value: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <Select.Trigger className="min-h-12 bg-field-background" data-testid="skills-local-runtime-select">
        <Select.Value>
          <span className="truncate font-semibold">{selectedRuntime ? runtimeLabel(selectedRuntime) : "Choose runtime"}</span>
        </Select.Value>
        <Select.Indicator />
      </Select.Trigger>
          ...
          {runtimes.map((runtime) => (
            <ListBox.Item key={runtime.id} id={runtime.id} textValue={runtime.name}>
              <div className="flex min-w-0 items-center gap-2 py-1">
                ...
                <Chip size="sm" variant="soft" color={runtime.status === "connected" ? "success" : "default"}>{runtime.status ?? "ready"}</Chip>
...
export function normalizeSkills(payload: unknown): SkillConfig[] {
  const rows = Array.isArray(payload)
    ...
    : isRecord(payload) && Array.isArray(payload.skills)
      ? payload.skills
      : [];
  return sortSkills(rows.map(normalizeSkill).filter((skill): skill is SkillConfig => skill !== undefined));
}
...
export function normalizeRuntimeLocalSkillList(payload: unknown): RuntimeLocalSkillConfig[] {
  ...
  return rows.flatMap((raw) => {
    ...
    return [{
      ...
      sourcePath: typeof raw.sourcePath === "string" ? raw.sourcePath : typeof raw.source_path === "string" ? raw.source_path : "",
...
export async function createSkill(fetchImpl: typeof fetch, input: SkillWriteInput): Promise<SkillConfig> {
  return writeSkill(fetchImpl, "/skills", "POST", input);
}

export async function updateSkill(fetchImpl: typeof fetch, skillId: string, input: SkillWriteInput): Promise<SkillConfig> {
...
export async function fetchRuntimeLocalSkills(fetchImpl: typeof fetch, runtimeId: string): Promise<RuntimeLocalSkillsResponse> {
  const response = await fetchImpl(`/runtimes/${encodeURIComponent(runtimeId)}/local-skills`, {
    method: "GET",
    credentials: "same-origin",
    headers: { accept: "application/json" }
  });
  if (!response.ok) throw await skillApiError(response, "Fetch local runtime skills failed");
  return normalizeRuntimeLocalSkillsResponse(await response.json());
}

export async function importRuntimeLocalSkill(fetchImpl: typeof fetch, runtimeId: string, input: { readonly skillKey: string; readonly name?: string; readonly description?: string }): Promise<SkillConfig> {
  return writeSkill(fetchImpl, `/runtimes/${encodeURIComponent(runtimeId)}/local-skills/import`, "POST", input);
}

export async function fetchSkillDetail(fetchImpl: typeof fetch, skillId: string): Promise<SkillConfig> {
  const response = await fetchImpl(`/skills/${encodeURIComponent(skillId)}`, {
    method: "GET",
    credentials: "same-origin",
    headers: { accept: "application/json" }
  });
  if (!response.ok) throw await skillApiError(response, "Fetch skill failed");
  const skill = normalizeSkill(await response.json());
  if (!skill) throw new Error("Skill response did not include a skill.");
  return skill;
}
...
export async function deleteSkill(fetchImpl: typeof fetch, skillId: string): Promise<void> {
  ...
  if (!response.ok) throw await skillApiError(response, "Delete skill failed");
}
...
function SkillEditorModal({
  ...
  const canSave = draft !== undefined && !readOnly && draft.name.trim().length > 0 && draft.description.trim().length > 0 && draft.content.trim().length > 0;
  ...
  const selectedContent = selectedPath === SKILL_MD_PATH ? draft?.content ?? "" : selectedSupportingFile?.content ?? "";
  ...
  const updateSelectedContent = (content: string) => {
    ...
    onChange({ ...draft, files: draft.files.map((file) => file.path === selectedPath ? { ...file, content } : file) });
  };
  ...
  const addSupportingFile = () => {
    ...
    if (path.length === 0 || path.toLowerCase() === "skill.md" || draft.files.some((file) => file.path === path)) return;
    onChange({
      ...draft,
  ...
  return (
    <Modal.Backdrop isOpen={draft !== undefined} onOpenChange={(open) => { if (!open) onClose(); }}>
      <Modal.Container size="lg">
        <Modal.Dialog className="sm:max-w-[860px]" aria-label="Skill editor">
            ...
            {draft ? (
              <div className="grid gap-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  <TextField value={draft.name} onChange={(value) => onChange({ ...draft, name: value })} isReadOnly={readOnly}>
                    <Label className="text-sm font-semibold">Name</Label>
                    <Input placeholder="skill-name" data-testid="skills-name-input" />
                  </TextField>
                  <TextField value={draft.description} onChange={(value) => onChange({ ...draft, description: value })} isReadOnly={readOnly}>
                    <Label className="text-sm font-semibold">Description</Label>
                    <Input placeholder="What this skill helps with" data-testid="skills-description-input" />
                  </TextField>
                </div>
                ...
                <div className="grid min-h-[420px] gap-4 lg:grid-cols-[260px_1fr]">
                  <div className="grid content-start gap-3 rounded-xl border border-border bg-surface p-3">
                    ...
                    <div className="grid gap-1">
                      ...
                      {draft.files.slice().sort((a, b) => a.path.localeCompare(b.path)).map((file) => (
                        <Button key={file.path} size="sm" variant={selectedPath === file.path ? "secondary" : "ghost"} className="justify-start truncate ah-mono" onPress={() => selectPath(file.path)}>
                          {file.path}
                        </Button>
                      ))}
                    </div>
                  ...
                  <div className="grid content-start gap-3">
                    {selectedPath !== SKILL_MD_PATH ? (
                      <div className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
                        <TextField value={selectedPath} onChange={renameSelectedFile} isReadOnly={readOnly}>
                    ...
                    <TextField value={selectedContent} onChange={updateSelectedContent} isReadOnly={readOnly}>
                      ...
                      <TextArea className="min-h-[340px] ah-mono text-xs" placeholder={selectedPath === SKILL_MD_PATH ? DEFAULT_SKILL_CONTENT : "File content"} data-testid="skills-content-input" />
                    </TextField>
                  </div>
                </div>
                {error ? <p className="text-xs text-danger" role="alert">{error}</p> : null}
          ...
          <Modal.Footer className="gap-2">
            ...
            {!readOnly ? <Button variant="primary" isPending={saving} isDisabled={!canSave || saving} onPress={onSave}>Save</Button> : null}
          </Modal.Footer>
        </Modal.Dialog>
      </Modal.Container>
    </Modal.Backdrop>
...
async function writeSkill(fetchImpl: typeof fetch, path: string, method: "POST" | "PUT", input: unknown): Promise<SkillConfig> {
  ...
  if (!response.ok) throw await skillApiError(response, `${method} skill failed`);
  const skill = normalizeSkill(await response.json());
  if (!skill) throw new Error("Skill response did not include a skill.");
  return skill;
}
...
async function skillApiError(response: Response, fallback: string): Promise<Error> {
  ...
  if (isRecord(payload)) {
    if (typeof payload.message === "string" && payload.message.length > 0) return new Error(payload.message);
...
function normalizeSkill(raw: unknown): SkillConfig | undefined {
  ...
  const rawFiles = isRecord(raw) && Array.isArray(raw.files)
    ? raw.files
