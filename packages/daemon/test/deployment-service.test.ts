...
describe("DeploymentService", () => {
  ...
  test("preview-url deployment becomes ready with a 30 minute expiry URL", async () => {
    ...
    expect(deployment).toMatchObject({ status: "ready", url: expect.stringContaining(`/deployments/${deployment.id}/preview`) });
    expect(currentDatabase().sqlite.prepare("SELECT status, url, expires_at FROM deployments WHERE id = ?").get(deployment.id)).toMatchObject({ status: "ready", expires_at: now + 30 * 60 * 1000 });
  ...
  test("static-site, source-zip, and container-export write downloadable files", async () => {
    ...
    expect(readFileSync(await service.downloadPath(sourceZip.id) ?? "", "utf8")).toContain("index.html");
    const dockerExport = readFileSync(await service.downloadPath(containerExport.id) ?? "", "utf8");
    expect(dockerExport).toContain("Dockerfile");
