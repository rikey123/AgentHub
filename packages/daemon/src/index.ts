...
async function route(ctx: RouteContext): Promise<void> {
  ...
  if (ctx.req.method === "GET" && url.pathname === "/rooms") return json(ctx.res, 200, { rooms: all(ctx.database, "SELECT * FROM rooms WHERE id NOT LIKE 'workflow:%' ORDER BY created_at ASC") });
  ...
  if (ctx.req.method === "GET" && parts[0] === "rooms" && parts.length === 1) return json(ctx.res, 200, { rooms: all(ctx.database, "SELECT * FROM rooms WHERE id NOT LIKE 'workflow:%' ORDER BY created_at ASC") });
