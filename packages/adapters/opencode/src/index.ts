...
import { AdapterBridge, buildRunPrompt, buildRunPromptAttachments, persistAssistantPublicMessage, type AdapterArtifactFSBoundary, type RoomMcpServer, type RunLifecycleService, type RunRow } from "@agenthub/orchestrator";
...
export class OpenCodeACPAdapter extends ACPAdapter {
  ...
  async runManaged(run: RunRow): Promise<void> {
    ...
    this.sendPrompt(session.id, this.promptMessageFromRun(run));
  }
  ...
  private persistAssistantMessageEnd(runId: string, messageId: string, text: string): void {
    ...
    if (db === undefined || eventBus === undefined || lifecycle === undefined) return;
