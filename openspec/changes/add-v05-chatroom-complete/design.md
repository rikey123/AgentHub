...
- OpenCode 团队已有 `@zed-industries/claude-agent-acp` / 类似 `@opencode/acp-bridge` 上游 bridge（待 [DECISION-NEEDED-V05-1] 确认 npm package 名），优先复用，不自己 scrape stdout。
...
| ID | 主题 | 推荐（已采纳） | 备选 | 状态 |
|---|---|---|---|---|
| V05-1 | OpenCode ACP bridge npm package | 待调研，候选 `@opencode-ai/acp` / `opencode-acp-client` / 自维护 fork | 自己 scrape stdout（已被 D8 否决） | **开工前调研定** |
...
| V05-5 | builder-opencode 模板默认 model | 待 OpenCode 默认确认 | 跟 OpenCode CLI 默认 | **开工前调研定（跟 OpenCode CLI 默认）** |
