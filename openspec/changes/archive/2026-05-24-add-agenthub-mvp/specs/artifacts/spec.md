...
> **Capability 概览**
>
> Artifact 是 Agent 产出的"可操作产物"——Diff、文件、预览、文档、终端日志、部署。MVP 重点：DiffCard（Monaco Diff）+ 文件附件 + Preview 占位；**Agent 不直接改文件**，只产出 patch，用户批准后系统应用（D13）。
>
> **Goals / Non-Goals**
> - G：用户始终是真理终点：所有写盘前要先成 Diff Card → 用户 accept → 应用。
> - G：Diff 应用支持回滚（保留 pre/post hash）。
> - G：终端输出能被引用（artifact `type=terminal`）。
> - NG：MVP 不做完整 Preview / Deploy（type 字段占位，行为 501）。
> - NG：MVP 不做 Artifact 版本树 UI（仅时间线）。

### Requirement: Artifact 数据模型

The system SHALL persist Artifact and ArtifactFile as below.

```ts
type ArtifactType = "diff" | "file" | "preview" | "document" | "terminal" | "deployment"
type ArtifactStatus = "draft" | "reviewing" | "accepted" | "applying" | "applied" | "rejected" | "failed"
...
type Artifact = {
  ...
  taskId?: string
  runId?: string
  messageId?: string                // 关联到聊天流中哪条消息的卡片
  type: ArtifactType
  title: string
  status: ArtifactStatus
  createdBy: string                 // agentId 或 userId
  createdAt: number
  updatedAt: number
  appliedAt?: number
}
...
type ArtifactFile = {
  ...
  oldContent?: string               // 应用前内容（应用时填充）
  newContent?: string               // 应用后预期内容
  patch?: string                    // unified diff
  additions: number
  deletions: number
  fileStatus: "added" | "modified" | "deleted"
  oldSha256?: string                // 用于冲突检测
  appliedState?: "original" | "new" | "unknown"   // 仅在 status='applied' / 'failed' 后有意义；recovery_required 时必填
}
```
...
CREATE TABLE artifact_files (
  ...
  old_sha256      TEXT,
  applied_state   TEXT,                              -- 'original' | 'new' | 'unknown'，apply/failed 后写入
  PRIMARY KEY (artifact_id, path)
);
```

#### Scenario: 写入 artifact + 多个文件

- **WHEN** Agent 通过 `room.publish_artifact { type: "diff", title: "Refactor auth", files: [{ path: "src/auth.ts", patch }, { path: "src/auth.test.ts", patch }] }` 提交
- **THEN** daemon 写一行 artifacts + 两行 artifact_files；返回 artifactId；event payload 引用同一 artifactId

#### Scenario: artifact 关联 message

- **WHEN** Agent 在某条 assistant 消息内嵌 DiffCard 提交 artifact
- **THEN** artifacts.message_id 设为该 message id，便于 UI 从 Card 跳回原消息
...
The system SHALL expose the following HTTP routes for artifact management; all mutating routes MUST emit corresponding durable events.

```
GET    /artifacts?roomId=&taskId=&status=
GET    /artifacts/:id
POST   /artifacts                       # 创建（一般由 adapter 经 MCP 触发，但 API 公开）
POST   /artifacts/:id/review            # 进入 reviewing
POST   /artifacts/:id/apply             # accept + 立即应用
POST   /artifacts/:id/reject
POST   /artifacts/:id/revert            # 仅 status=applied
GET    /artifacts/:id/files             # 列出文件
GET    /artifacts/:id/files/:path       # 单文件内容（diff / file content）
```

#### Scenario: 列出 reviewing 中的 artifact

- **WHEN** UI `GET /artifacts?roomId=r_1&status=reviewing,draft`
- **THEN** 返回当前 Room 内未决的 artifact 数组（按 createdAt 降序）
