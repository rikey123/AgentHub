...
> **Capability 概览**
>
> `event-system` 定义了"事件协议是什么、怎么落库、SSE 怎么写"。`bus-runtime` 定义"运行时这些事件怎么流转 + 命令怎么进入 + Run 怎么排队 + 失败怎么重试 + 谁订阅谁"。
>
> 两者关系：
>
> ```
> event-system   = 协议 + 存储 (DTO 层)
> bus-runtime    = Command Bus + Outbox + RunQueue + Handler/Projector + DLQ + 订阅图谱 (流转层)
> ```
>
> 本 capability 同时也是反压、事务边界、回放语义的"单一真相源"——其它 capability 出现冲突时以本 spec 为准。
>
> **Goals / Non-Goals**
> - G：Command 与 Event 在 API、表、bus 三个层面都被显式区分，不能混。
> - G：`domain state` + `events` 行 + `outbox` 行在同一 SQLite 事务中提交，崩溃恢复时不丢事件。
> - G：每个 durable handler 有自己的游标 + 失败重试 + 死信队列，daemon 重启可补处理漏的事件。
> - G：SSE 客户端有显式 buffer 上限和反压策略；Debug Panel 不拖慢主流程。
> - G：每个内部模块有明确的"发布 / 订阅"列表，bus 是模块间唯一通信方式（少数同步 query 例外）。
> - NG：MVP / 后续阶段都不引入 NATS / Kafka / Redis Streams（路线红线 D32：永不做云端 / 团队云端）。EventBus 接口与 in-process PubSub 解耦的真正动机是 V1.3 `plugin-system` 的事件订阅 API（插件按 visibility 过滤订阅 durable 事件，必须经过隔离边界），而不是为切外部 broker。
> - NG：MVP 不做跨进程 / 跨机器 bus（仅单 daemon 进程）。
>
> **Definitions**
> - **Command**：客户端或内部模块对系统的请求（"做某事"）。可被校验、可失败、可拒绝。
> - **Event**：事实陈述（"某事已发生"）。durable event 不可撤销（只能用补偿事件取消）。
> - **Outbox**：和 domain 表在同一事务里写入的待发箱表，由后台 Dispatcher 异步推进到 PubSub / SSE。
> - **Handler**：订阅 durable event 并产生副作用（创建 Run、写状态、调外部）的模块。
> - **Projector**：订阅 event 流维护一个只读视图（前端 view model 或后端缓存）。Projector 是无副作用的特殊 handler。
> - **RunService**：`CancelRun` Command 的 handler；负责入参校验、调 `RunLifecycleService.markCancelling` + 同步驱动 AdapterManager.cancelRun；本身**不**直接 UPDATE runs。**MVP 没有 StartRun Command**：`agent.run.queued` 唯一来源是 WakeAgent handler（详见 `Command 与 Event 显式区分` 与 `RunQueue 是 bus 的一条命名队列`）。
> - **RunLifecycleService**：`runs` 表的**唯一写入入口**。所有状态推进（create / markWaiting / markClaimed / markStarting / markRunning / markWaitingPermission / markCancelling / complete / fail / cancelFinalized / updateSessionState）必须经过它。每个**会发 durable event 的**方法在**单事务**内：① UPDATE runs ② INSERT 对应 durable event 到 events ③ INSERT outbox。RunQueue Worker、AdapterBridge、CancelRun handler、WakeAgent handler 都通过它完成 runs 状态推进，**禁止裸写 runs 表**。
> - **RunQueue Worker**：订阅 `agent.run.queued` 的 durable handler；负责锁矩阵调度、调用 `RunLifecycleService.markWaiting / markClaimed / markStarting`、调用 AdapterManager；**自身不写 runs**。
> - **WakeAgent handler**：模型调用的**唯一**入口。订阅 `WakeAgent` Command（origin='internal'），在 IMMEDIATE 事务内：① activeWakes guard ② 原子 claim mailbox（详见 `orchestrator/Mailbox 原子认领`）③ 调 `RunLifecycleService.create(...)` 创建 Run（service 自身在同事务 INSERT runs(queued) + INSERT events(agent.run.queued) + INSERT outbox）。`RetryRun` / `ConsumePendingTurn` handler 内部转 dispatch WakeAgent；不再走"StartRun → 创建 Run"的旧路径。
> - **AdapterBridge**：单个 adapter session 的"事件翻译器"。订阅 adapter 自身的 `Stream<AdapterEvent>`（in-process，非 durable），翻译为 durable 域事件——但所有 `agent.run.*` 推进必须通过 `RunLifecycleService` 的对应方法（complete / fail / cancelFinalized），不直接 publish；其它派生事件（`tool.call.*` / `subagent.*` / `file.changed` / `context.snapshot`）由 AdapterBridge 自身在事务内 publish。Adapter 自己**不**直接发 durable event。
> - **RunQueue**：Run 调度的逻辑队列。运行时状态来源是 `runs.status IN ('queued','waiting')`，不依赖 event 字段。
