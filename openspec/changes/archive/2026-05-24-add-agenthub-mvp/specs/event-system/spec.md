...
> **Capability 概览**
>
> AgentHub 的事件**协议 + 存储**层。三个核心：① 统一 Envelope 含版本与因果链；② 9 大类事件协议；③ durable / ephemeral 两层（Event Store + Effect PubSub）。
>
> **与 `bus-runtime` 的边界**：本 capability 只定义"事件长什么样、怎么落库、SSE 帧怎么写"。运行时怎么流转（Command Bus / Outbox / handler 游标 / 重试 / RunQueue / 反压 / 订阅图谱）见 `bus-runtime`。两者不能重复也不能矛盾——本 spec 的 seq / durability 是 `bus-runtime` 反压、cursor、回放语义的基石。
>
> **Goals / Non-Goals**
> - G：所有 durable 事件可重放，schema 可演进。
> - G：高频 ephemeral 事件不打挂 SQLite。
> - G：客户端 cursor 重连不丢 durable，可丢 ephemeral。
> - NG：永不引入 Kafka / NATS / 外部 broker（路线红线 D32：单机本地产品）。
> - NG：MVP 不做事件压缩 / 归档（V1.x，规模上来后再视需要）。
> - NG：本 spec **不**定义 Command、handler 重试、RunQueue、订阅图谱——这些在 `bus-runtime`。
>
> **Definitions**
> - **Envelope**：所有事件的统一封装，详见 Data Model。
> - **traceId**：一次用户请求的完整链路 id。
> - **causationId**：触发当前事件的上一事件 id。
> - **correlationId**：一组相关事件的归属 id（如同一 Run 内的所有事件）。
