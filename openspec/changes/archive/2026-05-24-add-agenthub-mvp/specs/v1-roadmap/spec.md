...
> **Capability 概览**
>
> 这份 spec **不实现任何功能**，只承诺 V0.5 / V1.0 / V1.x 各阶段能力的接口契约和扩展点，让 MVP 不写死、未来不破坏。本文 spec 的"Acceptance"是"接口存在且 MVP 没有阻塞后续接入"，不是"功能可用"。
>
> **路线红线（与 design.md D32 对齐）**：
>
> - **不做** SaaS / 云端 / Postgres / Redis / WebSocket Hub / 多用户认证 / 团队云端 / 移动 Native Client / Marketplace。
> - 所有 V0.5+ 能力都在**单机本地** daemon 内实现；任何看似需要云端的特性（如多端审批）必须能用本地 daemon 暴露给 LAN/反向代理满足，不引入云后端。
>
> **Goals / Non-Goals**
> - G：MVP 实现的内核（事件协议、Adapter 接口、Context Ledger、Permission Engine、Bus Runtime）允许下列后续能力以 adapter / plugin / view-projection 形式接入而不改内核。
> - G：所有未实现入口在 MVP 是 stub（detect 返回 []、API 返回 501、UI 隐藏）但接口签名与未来一致。
> - NG：本 spec 不要求实现任何后续行为；每个阶段有独立 change 提案。
>
> **阶段总览**：
>
> | 阶段 | 主题 | 占位能力 |
...
> | **V1.1** | 协作可视化 | `task-board`、`collab-visualization` |
> | **V1.2** | 上下文真生效 | `skill-system`、`bm25-recall`、`vector-search`、`memory-gateway` |
> | **V1.3** | 生态打开 | `plugin-system`、`langgraph-adapter`、`a2a-server`、`a2a-client` |
> | **V1.4** | 多端适配 | `desktop-shell-tauri`、`responsive-web`、`deployment-docker` |
> | **V1.5** | 高级编排 | `war-room-mode`、`permission-dsl` |
>
> 后续每个阶段都将以独立 change（如 `add-v0.5-chatroom-complete` / `add-v1.0-orchestration` / `add-v1.1-collab-views` …）展开成完整 spec。本 spec 仅锁定接口与扩展点。
