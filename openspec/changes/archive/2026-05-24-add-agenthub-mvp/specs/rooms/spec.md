...
> **Capability 概览**
>
> Room 是 AgentHub 的群聊容器。MVP 实现 Solo + Assisted 两种模式；Team + Squad 留 V1.0；War Room 留 V1.5。本 capability 定义 Room 实体、参与者矩阵、唤醒规则、群聊纪律。
>
> **Goals / Non-Goals**
> - G：用户能像建微信群一样新建房间，邀请 1 个 Primary + 多个 Observer Agent。
> - G：群聊纪律执行器拦截 Observer 的"自由发言"，强制走敲门 / mailbox / draft context。
> - NG：MVP 不实现 Team Mode（V1.0 leader 调度多 teammate）/ Squad（V1.0）/ War Room（V1.5）。
> - NG：MVP 不做房间分享 / 邀请链接（路线红线 D32：单机本地，无跨用户分享场景；本机/LAN 访问通过 token + 反向代理满足）。
>
> **Definitions**
> - **Room**：一个会话上下文容器，含参与者、消息流、上下文账本、任务板。
> - **Mode**：房间运行模式，决定参与者权限矩阵。
> - **Participant**：用户或 Agent 在 Room 中的角色化身。
> - **Presence**：Agent 在 Room 中的运行时状态（offline / observing / active / working / waiting_approval / knocking / blocked）。
