# squad-mode (V1.0 delta)

> **参考来源**：
> - **AionUi**（借产品模式）：
>   - `src/process/channels/`：多 agent 协作的消息路由模式——Leader 接收用户消息后路由给合适的 teammate，teammate 完成后反馈给 Leader。
> - **multica**（仅借模式）：
>   - `server/internal/handler/issue_child_done.go`：子任务完成判定 + 父级回顾逻辑——Squad 的 Leader wake 机制沿用此模式。
>   - `server/internal/issueguard/`：防止重复创建 / parent depth 限制——Squad delegation 的防循环规则借鉴此模式。
> - **总线契约**：
>   - 写路径：`room.delegate { expectsReview: false }` → INSERT tasks（轻量 Task）+ dispatch WakeAgent → emit `task.delegation.created`（durable, visibility=both）；teammate Run 终结 → Task 自动 completed → emit `task.delegation.completed`（durable, visibility=both）
>   - 读路径：前端 projector 订阅 `task.delegation.created/completed`（visibility=both），更新 Side Panel Tasks tab + 主流 brief
>   - 失败路径：teammate Run fail → Task status=blocked/cancelled → emit `task.status.changed`（durable, visibility=both）
