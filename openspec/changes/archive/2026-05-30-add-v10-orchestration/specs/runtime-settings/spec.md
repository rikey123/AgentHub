# runtime-settings (V1.0 delta)

> **参考来源**：
> - **AionUi**（Apache-2.0，可代码级复刻）：
>   - `src/renderer/pages/settings/AgentSettings/{LocalAgents,InlineAgentEditor,AgentCard}.tsx`：Local Agents 检测状态 / 自定义 Agent command/args/env / test connection / JSON 高级编辑——这是 V1.0 Runtimes 设置页的直接参考，可代码级移植。
>   - `src/renderer/hooks/agent/{useDetectedAgents,useMultiAgentDetection,useAgentReadinessCheck}.ts`：Runtime 就绪检测 + 用户提示安装哪个 CLI。
> - **总线契约**：
>   - 写路径：`POST/PATCH/DELETE /runtimes` → UPDATE runtimes 表 → emit `runtime.detected/updated/removed`（durable, visibility=detail）
>   - 读路径：**REST-only**；Settings UI 通过 `GET /runtimes` 初始化；test connection 通过 `POST /runtimes/:id/test` 同步或 job polling 返回；不订阅 SSE
