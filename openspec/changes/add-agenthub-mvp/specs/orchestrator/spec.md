...
> **Capability 概览**
>
> Orchestrator 是群聊纪律执行器 + Run 调度器。MVP 只实现 Solo + Assisted 模式，不做完整 DAG 任务拆解。Mailbox 是 durable inbox，让 Observer Agent 给 Primary 留言，避免内存级 EventEmitter 在 daemon 重启后丢消息。Room MCP Tools 让 Agent 主动查询房间状态而不是被强制塞进 prompt。
>
> **Goals / Non-Goals**
> - G：单聊 / 多 Agent 旁听场景的 Run 调度稳定。
> - G：Mailbox 落表，daemon 重启不丢留言。
> - G：Agent 通过 Room MCP Tools 查询动态状态，prompt 体积不爆炸。
> - NG：MVP 不实现 Team Mode 的 Leader 调度逻辑（V1.0）。
> - NG：MVP 不实现 DAG 任务拆解（Task.dependencies 字段保留，MVP 调度器忽略；V1.2 collab-visualization Dependency 视图消费）。
> - NG：MVP 不实现 Squad 路由（V1.0）/ War Room（V1.5）。
...
2. `@<agentName>` 解析成 mention list；mention 中的 agent SHALL 临时进入 `active` 并被调度（与 primary 串行；@agent 顺序按 mention 在文本中出现顺序），通过 `WakeAgent { reason: "user_mention" }`。
...
#### Scenario: 用户 @ 多个 Agent

- **WHEN** Assisted Room 中用户消息 `"@security 检查这段，@reviewer 也看下" + DiffCard`
- **THEN** Orchestrator 顺序触发 security run → reviewer run（不并行；保持简单），primary 不被本条消息触发（用户没 @ primary）
...
The system SHALL parse mentions in user messages by matching `@<agentName>` where agentName is the kebab-case `id` of an Agent participant in the room; matches MUST be at word boundaries.
...
The system SHALL throttle agent status line updates (`agent.status_line.updated` ephemeral events) to at most 1 visible update per 30 seconds per Agent per Room.

实际写 events / publish 次数不限，UI 渲染节流。

#### Scenario: 频繁状态更新

- **WHEN** Agent 在 60 秒内 emit 20 条 `agent.status_line.updated`
- **THEN** SSE 客户端 projector 只更新 view model 2 次（30 秒一帧）
