...
> **Capability 概览**
>
> Intervention（敲门介入）是 AgentHub 与"普通多 Agent"的最大差异之一：让 Observer Agent 在看到主 Agent 即将走偏时举手示警，让用户决定是否打断 / 注入指引。完整状态机：`requested → pending_user_decision → approved / ignored / rejected / snoozed → injected → resolved → closed`。
>
> **Goals / Non-Goals**
> - G：敲门成本低（一条 MCP tool call），用户裁决直观（4 个 action）。
> - G：approved 后自动通过 adapter 注入；按 adapter manifest 的 `injectionMode` 决定时机。
> - G：snoozed 可重新激活；ignored / rejected 不再激活。
> - NG：MVP 不做基于规则的"自动批准"（V1）。
> - NG：MVP 不做敲门优先级队列 / 多 Agent 同时敲门时的合并（仅去重，按 createdAt 排序）。
