...
> **Capability 概览**
>
> AgentHub 的本地服务进程。HTTP 外壳 = Hono；异步内核 = Effect（PubSub / Stream / Layer / Scope）。提供：① OpenAPI 3.1 接口 + 自动生成 SDK；② SSE `/event` 主事件流；③ 多客户端连接（Web / CLI / 桌面 / 移动）；④ 优雅启停与生命周期管理；⑤ 默认绑定 `127.0.0.1`。
>
> **Goals / Non-Goals**
> - G：单一可执行 daemon，能 `agenthub start` 起来即可被 Web UI 连。
> - G：SSE 重连时通过 cursor 重放遗失事件。
> - G：daemon 进程崩溃后所有 durable 状态可由 SQLite 还原。
> - NG：永不做 daemon 集群 / 多副本（路线红线 D32：单机本地 daemon，并发上限通过队列 + 锁矩阵控制）。
> - NG：MVP 不做 daemon 自动升级；V1.x 视需要评估**本地** updater（不是云端推送系统）。
>
> **Definitions**
> - **Workspace**：用户的项目目录（含 `.agenthub/` 子目录），daemon 内的逻辑租户。
> - **Session**：一次 SSE 客户端连接（含 cursor）。
> - **Run**：一次 Agent 执行（详见 `agents` capability）。
