...
> **Capability 概览**
>
> Permission Engine = `ask` / `allow` / `deny` 三态决策器 + Effect Deferred 异步审批。覆盖 5 类资源：file / shell / tool / context / agent。MVP 必须把所有外部副作用（写文件、跑命令）走 Permission Engine，不允许 adapter 绕过。
>
> **Goals / Non-Goals**
> - G：用户审批疲劳低（项目内 / 项目外 / 敏感三档 + "本项目总是允许"）。
> - G：Agent 不能凭 Capability 声明就做事，必须配套 PermissionProfile。
> - G：审批可挂起、超时、恢复（Effect Scope + Deferred）。
> - NG：MVP 不做 OS 级别的 sandbox（如 macOS App Sandbox / Windows AppContainer）—— 仅靠 Permission Engine + 子进程隔离。
> - NG：MVP 不做基于角色的 RBAC（仅有"用户"和"Agent"两类主体）。
