...
> **Capability 概览**
>
> daemon 默认 `127.0.0.1` + 远程 token、API key 走 OS keychain、敏感文件白名单、prompt injection 防护、子进程隔离。**这些不是"可选最佳实践"，是不可破的红线**。设计上明确："Agent 读到含 ignore previous instructions 的文件时不自动提升权限"。
>
> **Goals / Non-Goals**
> - G：本地默认免登录可用，但任何远程访问必须显式开启 token。
> - G：API key / token 不进 SQLite 明文。
> - G：Agent 不能通过文件内容欺骗系统提升权限。
> - NG：MVP 不做 OS 级 sandbox（macOS App Sandbox、Windows AppContainer）—— 仅靠子进程隔离 + Permission Engine。
> - NG：永不做端到端加密 / 多用户隔离（路线红线 D32：单机本地产品，无多用户场景）。
