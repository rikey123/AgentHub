...
- [x] **1.6** 跑共享契约基线验证后再作为 backend/web 的 merge base
...
- [x] **2.10** RoomList backend retrofit
...
- [x] **3.2** Artifact Versioning Service（文本 + binary）
...
- [x] **3.4** Artifact library/list API
...
- [x] **3.5** PPT Preview Bridge（ppt-preview-bridge.ts）
...
- [x] **3.6** PPT Proxy Route（/api/ppt-proxy/:port/*）
...
- [x] **3.7** @artifact / @workspace Context-Ref Resolver
...
- [x] **3.8** Pinned Messages Context Assembly 优先级
...
- [x] **3.9** Message Pin/Unpin backend retrofit
...
- [x] **3.10** Message actions backend verification
...
- [x] **3.12** Orchestrator Prompt 模板更新
...
- [x] **4.1** FeatureRail real navigation + HeroUI shell
...
- [x] **4.2** Contacts rail / panel + Agent Contact Directory
...
- [x] **4.3** Contact-first NewRoomDialog + Advanced participant config
  - 默认流程：先选联系人，再选模式
...
- [x] **4.4** PreviewCard / DocumentCard / PresentationCard UI anatomy
...
- [x] **4.5** ArtifactPreviewModal 升级为 Artifact Studio
...
- [x] **4.6** DeploymentCard（完整状态机 + 日志 UI）
...
- [x] **4.7** Input Composer token/pill model
...
- [x] **4.9** Settings → Deploy Providers
...
- [x] **4.10** Projector normalized state + handlers
