...
**成熟开源参考：**
- **bolt.diy**：AI 对话 → 代码产物（`<boltArtifact>` 流式解析）→ DeployAlert 两阶段状态（build → deploy）→ Vercel/Netlify/GitHub 多 provider 接入；`workbenchStore.addArtifact` 把产物和聊天分离的模式。
...
- **AionUi**：`open_file_preview(path)` tool-driven panel，`InlineAgentEditor`（name/avatar/command/args/test connection），右侧 preview panel 模型。
- **Golutra**：`ConversationDto`（`pinned` / `lastMessageAt` / `unreadCount`），`ensureDirectConversation`，room list 排序。
- **Dokploy**：BullMQ 队列驱动部署，WebSocket 实时日志（`tail -f` over SSH/local），`deployment` record schema（status / logPath / pid / finishedAt）。
...
- **新建对话选择 Agent**：新建 room 时展示 Agent Contact Directory 面板；选一个 Agent → `assisted` 单聊；选多个 → 选 `squad`/`team` 模式创建群聊；旧的 role/runtime picker 保留为"高级"折叠项。
...
- **Agent Contact Directory**：左侧新增 Contacts 面板；`GET /agents/contacts` 返回所有 `agent_bindings`（avatar / displayName / capabilities / status: available/busy/offline）；点击联系人发起"Start Chat"。
- **对话式创建自建 Agent**：联系人面板或 InputBox `/create-agent` 触发覆盖式向导（不跳转页面）；填写 name / avatar / system prompt / runtime / model config / skills；"Test Connection" 调 `POST /runtimes/:id/health`；保存后 Agent 进入联系人列表；参考：AionUi `InlineAgentEditor`。
...
- **容器构建**（`kind="container-build"`）：检测本机 `nixpacks` 或 `docker` CLI；优先 Nixpacks；无则降级为 container-export；WebSocket 实时日志流（`/deployments/:id/logs`）；参考：Dokploy `listen-deployment.ts`，bolt.diy DeployAlert 两阶段状态。
...
- `im-chat-core-completion`：会话搜索、新建对话选 Agent、置顶/最近活跃排序、多会话并行、消息操作六项、Pin 关键消息为长期上下文。
- `agent-contact-custom`：Agent Contact Directory（available/busy/offline 状态）、对话式向导创建自建 Agent（InlineAgentEditor + Test Connection）。
- `group-chat-orchestration`：@ 触发可见分派公告、成员短消息 + Artifact Card 分离、Orchestrator 最终汇总、失败降级可见。
- `artifact-message-cards`：PreviewCard / DocumentCard / PresentationCard / DiffCard / DeploymentCard 的 `message.part.added` 合约；live projector 更新。
...
- `messaging`：消息操作（Reply / Quote / Regenerate）后端支持；pinned message context assembly 优先级更新（复用 `messages.pinned_at`）。
...
- **Schema 迁移** `0019_v12.sql`：新表 `artifact_versions` / `deployments` / `deployment_providers` / `wake_outbox`；新列 `rooms.pinned_at` / `rooms.last_activity_at` / `artifacts.kind`（含 `presentation_pptx`）/ `tasks.last_unblocked_at` / `agent_bindings.avatar_url` / `agent_bindings.contact_name` / `agent_bindings.contact_description`；`deployments` 表含 `pid` / `started_at` / `finished_at` / `cancelled_at` / `artifact_version` / `last_error`。（`rooms.archived_at` 已存在于 `0001_init.sql`，不重复添加；`messages.pinned_at` 已存在于 `0013_messages_pinned.sql`，不重复添加。）
- **Protocol**：新增 EventCategory `"deployment"`；注册 V1.2 全部新事件（见下方 Event Registry Contract）。
