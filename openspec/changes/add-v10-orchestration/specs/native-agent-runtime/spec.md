# native-agent-runtime (V1.0 delta)

> **参考来源**：
> - **AionUi**（Apache-2.0，借产品边界不借代码）：
>   - `src/process/agent/aionrs/`：内置 agent runtime 的产品形态——装完即用 + 任意 API key + 文件读写 + tool calling。V1.0 Native Runtime 对齐此能力线，**不**对齐 Claude Code 内部代码智能。
> - **OpenCode**（参考模式）：
>   - `packages/opencode/src/provider/provider.ts:91`：`BUNDLED_PROVIDERS` + `sdk.languageModel(modelId)` 显式 provider 实例化——V1.0 Native Runtime 沿用此模式，禁止字符串 model ID。
>   - `packages/opencode/src/session/llm.ts:325`：`streamText` 接收已解析的 `language` model 实例，不接受字符串 modelId。
>   - `packages/llm/src/providers/openai-compatible-profile.ts:10`：DeepSeek / OpenRouter / Groq 等 baseURL profile。
> - **总线契约**：
>   - 写路径：NativeAgentAdapter 通过 AdapterBridge 发 `tool.call.requested` / `tool.call.completed` / `message.part.delta` / `agent.run.*`（与 ClaudeCodeAdapter / OpenCodeACPAdapter 一致）
>   - 读路径：前端 Run Detail Tools tab 通过 SSE `?view=detail&runId=<id>` 消费（visibility=detail 事件）；主流通过 `message.brief.published`（visibility=main）消费
>   - 失败路径：模型调用失败 → RunLifecycle.fail(failureClass) → emit `agent.run.failed`（durable, visibility=both）
