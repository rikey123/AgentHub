# role-system (V1.0 delta)

> **参考来源**：
> - **AionUi**（Apache-2.0，可代码级复刻）：
>   - `src/renderer/pages/settings/AssistantSettings/{AssistantListPanel,AssistantEditDrawer,AssistantAvatar}.tsx`：Role（Assistant）与 Agent 已经分开；AssistantListPanel 的列表 + 搜索 + 新建入口可直接移植；AssistantEditDrawer 的 prompt 编辑 + capabilities 多选 + avatar 选择是 V1.0 Role 编辑器的直接参考。
>   - `src/renderer/pages/settings/AgentSettings/PresetManagement.tsx`：内置 preset 管理（版本检测 + 用户覆盖保护）对应 V1.0 内置 Role 模板首启写入逻辑。
> - **multica**（仅借模式）：
>   - `server/internal/handler/issue.go`：assignee 与具体 agent_id 解耦的模式——multica Issue 的 assignee 是 user/team，不是具体 agent 实例；V1.0 Task 的 `assignee_role_id` 走同样思路。
> - **总线契约**：
>   - 写路径：`POST/PATCH/DELETE /roles` → UPDATE roles 表 → emit `role.created/updated/deleted`（durable, visibility=detail）
>   - 读路径：**REST-only**；Settings UI 通过 `GET /roles` 初始化，写后用 response 刷新；不订阅 SSE，不要求 projector handler
