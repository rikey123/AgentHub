# role-generator (V1.0 delta)

> **参考来源**：
> - **AionUi**（Apache-2.0，可代码级复刻）：
>   - `src/renderer/pages/settings/AgentSettings/InlineAgentEditor.tsx`：JSON 高级编辑 + "generate then edit" 模式——用户先生成草稿，再在编辑器里修改，最后保存。这是 V1.0 role-generator 的直接 UX 参考。
> - **multica**（仅借模式）：
>   - `server/cmd/multica/cmd_issue.go`：issue 创建强制用户确认——AI 自动写入会污染产品语义；V1.0 role-generator 沿用同样原则。
> - **总线契约**：
>   - 写路径：role 生成走 **REST job polling**，不进 EventBus；`POST /roles/generate → 202 { jobId }`；UI 每 500ms `GET /roles/generate/jobs/:jobId`；用户保存时 `POST /roles` → emit `role.created`（durable, visibility=detail）
>   - 读路径：**polling 路径**；UI 通过 `GET /roles/generate/jobs/:jobId` 轮询状态 + 增量 draft；不订阅 SSE，不 emit EventBus 事件
>   - 失败路径：job status=failed → `GET /roles/generate/jobs/:jobId` 返回 `{ status: "failed", error }` → UI 显示错误 + 引导手写；不 emit EventBus 事件
