...
- **单聊、协作、团队三类任务形态**：支持 `solo`、`assisted`、`team`、`squad` 房间模式，覆盖从单 Agent 快速任务到多 Agent 分工协作。
...
| `squad`    | team 的轻量扩展形态，保留协议兼容与高级配置入口             |
...
`.\agenthub.cmd web` 会检查 daemon 是否已启动；如果没有，会启动本地 daemon，并打开 Web 工作台。
...
```powershell
# daemon 生命周期
.\agenthub.cmd web            # 启动 daemon 并打开 Web 工作台
.\agenthub.cmd start          # 后台启动 daemon
.\agenthub.cmd status         # 查看 daemon 状态
.\agenthub.cmd stop           # 停止 daemon
.\agenthub.cmd doctor         # 环境自检
.\agenthub.cmd auth issue     # 签发远程/移动端访问 token
...
| v1.1 | Multi-Agent Complete             | Assisted / Team / Squad、selector、team dispatch、任务闭环  |
